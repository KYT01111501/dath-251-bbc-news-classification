...
def main():
    ...
    # ---- chọn model tốt nhất theo accuracy và lưu ----

    BASE = Path(__file__).resolve().parent  # lưu cùng thư mục script
    best = max(results, key=lambda r: r['accuracy'])  # hoặc chọn theo tên model bạn muốn
    best_model = best['model']

    joblib.dump(vectorizer, BASE / "tfidf_vectorizer.pkl")
    joblib.dump(best_model, BASE / "text_model.pkl")
    joblib.dump(label_encoder, BASE / "label_encoder.pkl")
    print("Saved: tfidf_vectorizer.pkl, text_model.pkl, label_encoder.pkl")
    ...
    with open('classification_results.html', 'w', encoding='utf-8', newline='') as f:
     f.write(html_report)
